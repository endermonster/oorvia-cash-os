    ...
    <div className="rounded-2xl border border-slate-700 bg-slate-900">
      ...
      >
        <div className="flex items-center gap-2">
          <span className="text-sm font-semibold text-slate-100">Campaign Attribution</span>
        ...
        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="2"
          strokeLinecap="round" className={`text-slate-500 transition-transform ${open ? 'rotate-180' : ''}`}>
          <path d="M4 6l4 4 4-4" />
        </svg>
      </button>
      ...
      {open && (
        <div className="border-t border-slate-700 px-5 pb-5">
          <div className="flex items-center justify-between py-3">
            <p className="text-xs text-slate-400">
              ...
              <span className="text-slate-500">
                Naming convention: <span className="font-mono text-slate-400">SKU - Campaign Type - V#</span>
                {' '}(e.g. <span className="font-mono text-slate-400">YB31 - Conversion - V1</span>)
              </span>
            </p>
            <button
              onClick={autoLink}
              className="ml-4 shrink-0 rounded-lg border border-slate-600 px-3 py-1.5 text-xs text-slate-300 hover:bg-slate-700 transition"
            >
              Auto-link by name
            </button>
          </div>
          ...
          {!loaded ? (
            ...
            <p className="text-sm text-slate-500 py-4">No campaigns found. Sync from Meta first.</p>
          ) : (
                  ...
                  {campaigns.map(c => (
                    <tr key={c.campaign_id} className="hover:bg-slate-800/40">
                      <td className="px-4 py-3">
                        <p className="text-slate-100">{c.campaign_name || '—'}</p>
                        <p className="text-slate-600 font-mono text-xs">{c.campaign_id}</p>
                      </td>
                      <td className="px-4 py-3 text-right text-slate-300">{fmtINR(c.total_spend)}</td>
                      <td className="px-4 py-3">
                        <select
                          value={c.sku || ''}
                          disabled={saving === c.campaign_id}
                          onChange={e => handleSkuChange(c.campaign_id, c.campaign_name, e.target.value || null)}
                          className="rounded-lg border border-slate-700 bg-slate-800 px-2 py-1.5 text-sm text-slate-100 focus:outline-none focus:ring-2 focus:ring-emerald-500 disabled:opacity-40"
                        >
...
export default function AdSpendPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <PageHeader
        ...
        actions={
          <>
            ...
            <button onClick={openAdd} className="rounded-lg bg-emerald-600 px-4 py-2 text-sm font-medium text-white hover:bg-emerald-500">
              + Add Spend
            </button>
          </>
        }
      ...
      {showForm && (
        <form onSubmit={handleSave} className="rounded-2xl border border-slate-700 bg-slate-900 p-5">
          <h3 className="text-sm font-semibold text-slate-100 mb-3">{editingEntry ? 'Edit Entry' : 'Add Ad Spend'}</h3>
        ...
        <div className="rounded-2xl border border-slate-700 bg-slate-900 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-800">
                {entries.length === 0 ? (
                  <tr><td colSpan={10} className="px-4 py-8 text-center text-slate-500">No ad spend entries this month.</td></tr>
                ) : entries.map((e) => {
                  ...
                  return (
                    <tr key={e.id} className="hover:bg-slate-800/60">
                      ...
                      <td className="px-4 py-3 text-right text-slate-400">{e.impressions?.toLocaleString('en-IN') || '—'}</td>
                      <td className="px-4 py-3 text-right text-slate-400">{e.clicks?.toLocaleString('en-IN') || '—'}</td>
                      <td className="px-4 py-3 text-right text-slate-300">{e.purchases || '—'}</td>
                      <td className="px-4 py-3 text-right text-slate-400">₹{cpl}</td>
                      <td className="px-4 py-3 text-right text-slate-400">₹{entryCA}</td>
                      <td className="px-4 py-3">
                        <div className="flex gap-1 justify-end">
                          <button onClick={() => openEdit(e)} className="text-xs text-slate-400 hover:text-slate-100 px-1.5 py-1 rounded hover:bg-slate-700">Edit</button>
                          <button onClick={() => handleDelete(e.id)} disabled={deletingId === e.id} className="text-xs text-red-400 hover:text-red-200 px-1.5 py-1 rounded hover:bg-slate-700 disabled:opacity-40">
                            {deletingId === e.id ? '…' : 'Del'}
                          </button>
                        </div>
                      </td>
