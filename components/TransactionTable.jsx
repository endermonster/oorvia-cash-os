...
const TYPE_STYLES = {
  income: 'bg-emerald-950 text-emerald-400',
  expense: 'bg-red-950 text-red-400',
  transfer: 'bg-yellow-950 text-yellow-400',
}
...
function RowMenu({ onEdit, onDelete, deleting }) {
  ...
  return (
    <div ref={ref} className="relative flex justify-end">
      ...
      {open && (
        <div className="absolute right-0 top-8 z-20 w-32 rounded-xl border border-slate-700 bg-slate-800 py-1 shadow-xl">
          <button
          ...
          <button
            ...
            disabled={deleting}
            className="flex w-full items-center gap-2 px-3 py-2 text-sm text-red-400 hover:bg-slate-700 disabled:opacity-40"
          >
            <svg width="13" height="13" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
      <div className="w-full max-w-lg rounded-2xl border border-slate-700 bg-slate-900 p-6 shadow-2xl">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-base font-semibold text-slate-100">Edit Transaction</h2>
              ...
              {slice.length === 0 ? (
                ...
                slice.map((t) => (
                  <tr key={t.id} className="hover:bg-slate-800/60">
                    ...
                    <td className="px-4 py-3 text-slate-200">{t.source}</td>
                    <td className="px-4 py-3 text-slate-300">{t.category || '—'}</td>
                    <td className="px-4 py-3">
                      <span className={`rounded-full px-2 py-0.5 text-xs font-medium capitalize ${TYPE_STYLES[t.type] || 'bg-slate-700 text-slate-300'}`}>
                        {t.type}
                      </span>
                    </td>
                    <td className={`px-4 py-3 text-right font-semibold whitespace-nowrap ${t.type === 'expense' ? 'text-red-400' : t.type === 'income' ? 'text-green-400' : 'text-slate-200'}`}>
